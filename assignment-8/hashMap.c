...
/*
	...
	HINT - use insertMap to re-hash the keys. Everything has been completed except the copy/re-hash.
...
void _reSizeTable(struct hashMap *h, int newCap) {
 ...
 /*
	...
	Concordance will manage the word count (value) - the hash is not concerned with counts

	You must monitor the load factor and resize (double size) when the load factor is greater than LOAD_FACTOR_THRESHOLD (.75)
	Use the tableLoad function - contains an explanation of load factor
 */
